...
- 🚧 **Phase 8 (Accounts) — Track B done, Track A pending.** Head-chef-only accounts (Sign in with Apple); guests stay anonymous by code. **Track B (code foundation, shipped to `main`, NOT OTA'd — rides the Apple rebuild):** migration `20260620120000_accounts_phase8.sql` adds nullable `cooks.user_id` + `kitchens.owner_user_id` (both `on delete set null`, so deleting an auth user unlinks rather than cascading away a live kitchen). New `_shared/auth.ts` `getUserId()` verifies a Supabase Auth access token via the admin client (null for anonymous calls). `create-kitchen` stamps `owner_user_id`/`user_id` when a host token is present, anonymous otherwise (verified: anon create still works). New `delete-account` function (auth-required; deletes the auth user, FK unlinks kitchens). Client: supabase client now has auth enabled (`persistSession`/`autoRefreshToken`) backed by a **chunking SecureStore/Keychain adapter** (`src/lib/secureStoreAdapter.ts` — Supabase session blobs exceed SecureStore's 2KB/value limit); new `AuthProvider` + `useAuth` (`src/lib/auth.ts`) hold the session with `signOut`/`deleteAccount` (`signInWithApple` is a documented stub); `api.invoke` gained an opt-in auth header used by `createKitchen` + `deleteAccount`; new `SettingsScreen` (account state, sign out, delete-account with a mid-cook guard) reachable from the Landing footer. **Track A (CODE done, on `main`, not built):** Phase 8 now ships **both Apple and Google** sign-in. Installed `expo-apple-authentication` + `@react-native-google-signin/google-signin`; `AuthProvider` has real `signInWithApple` + `signInWithGoogle` (`supabase.auth.signInWithIdToken`); `SignInButtons` (native Apple button + Google), `SignInScreen` (create-kitchen gate), Landing host-gate, Settings sign-in/out. Apple dashboard config done (Florian); **Google dashboard config + client IDs pending Florian.** **These native modules mean the app no longer runs in Expo Go — test via EAS build.** **Pre-build checklist:** (1) Florian sends Google Web+iOS client IDs; (2) fill `src/lib/authConfig.ts`; (3) set app.json google-signin `iosUrlScheme` to reversed iOS client id; (4) bump `version` → `1.1.0` + `OTA_VERSION`; (5) `eas build` → TestFlight. Full spec + checklist: DESIGN.md §11a.

### v2 — commercial pivot (decided 2026-06-20)

**v1 is complete and shipped. CookCrew is going commercial.** The active roadmap now lives in **DESIGN.md §11** ("v2 — commercial foundation"). Headlines: freemium model, iOS-first polished launch, build sequence = Phase 8 Accounts → 9 Cost/abuse+metering → 10 Freemium paywall → 11 Recipe history → 12 Multi-recipe → 13 Small wins (rejoin, scaling, timers, share extension). v3 = Android, push, disconnect auto-promotion, video parsing. Three commercial table-stakes (cost/abuse control, security hardening, legal/IAP) outrank feature work — see DESIGN.md §11 + decision log 2026-06-20. **Operational prereq before charging: move off Florian's dad's accounts** (Nick's own Apple Developer account, paid Supabase org, Anthropic billing).

### v2 deferral notes (detail; superseded-context where DESIGN.md §11 is now authoritative)

- **Disconnect detection + sous-chef auto-promotion** *(now v3)*. Original DESIGN.md §6 specced this; we trimmed it during Phase 6 (see decision log). For v1, sous chef is appointed manually and has delegate powers, but isn't auto-promoted if the head chef drops. If the head chef loses connection, the recipe is locked until they reconnect — recoverable but not graceful. Revisit if testing surfaces real pain.
...
- **v1 shipped; the active scope is now the commercial v2 foundation** (DESIGN.md
  §11, decided 2026-06-20). Build in the documented phase order (8 Accounts → 9
  Cost/abuse+metering → 10 Freemium paywall → 11 History → 12 Multi-recipe → 13
  Small wins). The three commercial table-stakes — cost/abuse control, security
  hardening, legal/IAP — outrank feature work; flag any request that reorders
  around them. v3 items (Android, push, disconnect auto-promotion, video parsing)
  are still fenced — confirm before building those.
